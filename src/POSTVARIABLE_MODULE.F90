MODULE POSTVARIABLE_MODULE
  ...
  CONTAINS
    ...
    SUBROUTINE CONSTRUCT(VARIABLE,CONFIG,EOS,ISTART,IEND,NSOLUTION)
      ...
      REAL(8) :: VAR
      INTEGER :: I,J,K,N,M,L,IO,NQQ,ITER
      ...
      DO L=1,VARIABLE%NSOLUTION
        ...
        DO M=0,VARIABLE%SOLUTION(L)%SIZE-1
          READ(IO) VARIABLE%SOLUTION(L)%DOMAIN(M)%RANK,VARIABLE%SOLUTION(L)%DOMAIN(M)%IMAX,VARIABLE%SOLUTION(L)%DOMAIN(M)%JMAX
          
          ALLOCATE(VARIABLE%SOLUTION(L)%DOMAIN(M)%PV(VARIABLE%NPV,VARIABLE%SOLUTION(L)%DOMAIN(M)%IMAX,VARIABLE%SOLUTION(L)%DOMAIN(M)%JMAX))
          ALLOCATE(VARIABLE%SOLUTION(L)%DOMAIN(M)%DV(VARIABLE%NDV,VARIABLE%SOLUTION(L)%DOMAIN(M)%IMAX,VARIABLE%SOLUTION(L)%DOMAIN(M)%JMAX))
          ALLOCATE(VARIABLE%SOLUTION(L)%DOMAIN(M)%TV(VARIABLE%NTV,VARIABLE%SOLUTION(L)%DOMAIN(M)%IMAX,VARIABLE%SOLUTION(L)%DOMAIN(M)%JMAX))
          
          READ(IO) (((VARIABLE%SOLUTION(L)%DOMAIN(M)%PV(N,I,J),N=1,VARIABLE%NPV),I=2,VARIABLE%SOLUTION(L)%DOMAIN(M)%IMAX),J=2,VARIABLE%SOLUTION(L)%DOMAIN(M)%JMAX)
          READ(IO) (((VARIABLE%SOLUTION(L)%DOMAIN(M)%TV(N,I,J),N=1,VARIABLE%NTV),I=2,VARIABLE%SOLUTION(L)%DOMAIN(M)%IMAX),J=2,VARIABLE%SOLUTION(L)%DOMAIN(M)%JMAX)
          READ(IO) ((((VAR,K=1,NQQ),N=1,VARIABLE%NPV),I=2,VARIABLE%SOLUTION(L)%DOMAIN(M)%IMAX),J=2,VARIABLE%SOLUTION(L)%DOMAIN(M)%JMAX)
          
          DO J=2,VARIABLE%SOLUTION(L)%DOMAIN(M)%JMAX
            DO I=2,VARIABLE%SOLUTION(L)%DOMAIN(M)%IMAX
              CALL EOS%DETEOS(VARIABLE%SOLUTION(L)%DOMAIN(M)%PV(1,I,J),VARIABLE%SOLUTION(L)%DOMAIN(M)%PV(4,I,J),VARIABLE%SOLUTION(L)%DOMAIN(M)%PV(5,I,J),VARIABLE%SOLUTION(L)%DOMAIN(M)%PV(6,I,J) &
                              ,VARIABLE%SOLUTION(L)%DOMAIN(M)%DV(:,I,J)) 
            END DO
          END DO
        END DO
    ...
    !CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
    FUNCTION GETPV(VARIABLE,L,M,N,I,J)
      IMPLICIT NONE
      CLASS(T_VARIABLE), INTENT(IN) :: VARIABLE
      INTEGER, INTENT(IN) :: L,M,N,I,J
      REAL(8) :: GETPV
      
      GETPV = VARIABLE%SOLUTION(L)%DOMAIN(M)%PV(N,I,J)
    END FUNCTION GETPV
    !CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
    FUNCTION GETTV(VARIABLE,L,M,N,I,J)
      IMPLICIT NONE
      CLASS(T_VARIABLE), INTENT(IN) :: VARIABLE
      INTEGER, INTENT(IN) :: L,M,N,I,J
      REAL(8) :: GETTV
      
      GETTV = VARIABLE%SOLUTION(L)%DOMAIN(M)%TV(N,I,J)
    END FUNCTION GETTV
    !CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
    FUNCTION GETDV(VARIABLE,L,M,N,I,J)
      IMPLICIT NONE
      CLASS(T_VARIABLE), INTENT(IN) :: VARIABLE
      INTEGER, INTENT(IN) :: L,M,N,I,J
      REAL(8) :: GETDV
      
      GETDV = VARIABLE%SOLUTION(L)%DOMAIN(M)%DV(N,I,J)
    END FUNCTION GETDV
    !CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
