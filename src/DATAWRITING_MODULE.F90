    ...
    SUBROUTINE CGNSWRITING(DATAWRITING,CONFIG,VARIABLE)
      ...
      DO N=1,VARIABLE%GETNSOLUTION()
        ...
        DO M=0,VARIABLE%GETSIZE(N)-1
          ...
          ALLOCATE(PV(VARIABLE%GETIMAX(N,M)-1,VARIABLE%GETJMAX(N,M)-1,VARIABLE%GETNPV()))
          ALLOCATE(DV(VARIABLE%GETIMAX(N,M)-1,VARIABLE%GETJMAX(N,M)-1,VARIABLE%GETNDV()))
          ALLOCATE(TV(VARIABLE%GETIMAX(N,M)-1,VARIABLE%GETJMAX(N,M)-1,VARIABLE%GETNTV()))
          ALLOCATE(A(VARIABLE%GETIMAX(N,M)-1,VARIABLE%GETJMAX(N,M)-1))
          
          DO J=2,VARIABLE%GETJMAX(N,M)
            DO I=2,VARIABLE%GETIMAX(N,M)
              DO L=1,VARIABLE%GETNPV()
                PV(I-1,J-1,L) = VARIABLE%GETPV(N,M,L,I,J)
              END DO
              DO L=1,VARIABLE%GETNDV()
                DV(I-1,J-1,L) = VARIABLE%GETDV(N,M,L,I,J)
              END DO
              DO L=1,VARIABLE%GETNTV()
                TV(I-1,J-1,L) = VARIABLE%GETTV(N,M,L,I,J)
              END DO
            END DO
          END DO
          A = DSQRT(DV(:,:,6))
          ...
          CALL CG_FIELD_WRITE_F(IFILE,1,VARIABLE%GETRANK(N,M)+1,INDEX_FLOW,RealDouble,'TEMPERATURE',PV(:,:,4),INDEX_FIELD,IER)
          CALL CG_FIELD_WRITE_F(IFILE,1,VARIABLE%GETRANK(N,M)+1,INDEX_FLOW,RealDouble,'Y1',PV(:,:,5),INDEX_FIELD,IER)
          CALL CG_FIELD_WRITE_F(IFILE,1,VARIABLE%GETRANK(N,M)+1,INDEX_FLOW,RealDouble,'Y2',PV(:,:,6),INDEX_FIELD,IER)
          ...
          IF(CONFIG%GETITURB().GE.-1) THEN
            CALL CG_FIELD_WRITE_F(IFILE,1,VARIABLE%GETRANK(N,M)+1,INDEX_FLOW,RealDouble,'K',PV(:,:,7),INDEX_FIELD,IER)
            CALL CG_FIELD_WRITE_F(IFILE,1,VARIABLE%GETRANK(N,M)+1,INDEX_FLOW,RealDouble,'OMEGA',PV(:,:,8),INDEX_FIELD,IER)
