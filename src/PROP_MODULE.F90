MODULE PROP_MODULE
  ...
  CONTAINS
    ...
    SUBROUTINE IAPWS97(PROP,RHO,T,LIQUID)
      ...
      REAL(8) :: TT,RR,MU0,MU1,MU2,LAM0,LAM1,LAM2
      INTEGER :: I,J
      
      TT = T/647.27D0
      RR = DMAX1(1.D-3,DMIN1(1332.4D0,RHO))/317.763D0
      
      MU0 = DSQRT(TT)/(PROP%PROP_COEFF%H(0) + PROP%PROP_COEFF%H(1)/TT + PROP%PROP_COEFF%H(2)/TT**2 &
                     + PROP%PROP_COEFF%H(3)/TT**3)

      MU1 = 0.D0
      DO I = 0,5
        DO J = 0,6
          MU1 = MU1 + PROP%PROP_COEFF%HH(I,J)*(1.D0/TT-1.D0)**I*(RR-1.D0)**J
        END DO
      END DO
      MU1 = DEXP(RR*MU1)
      MU2 = 1.D0
      ...
      LAM0 = DSQRT(TT)/(PROP%PROP_COEFF%L(0) + PROP%PROP_COEFF%L(1)/TT + PROP%PROP_COEFF%L(2)/TT**2 &
                      + PROP%PROP_COEFF%L(3)/TT**3)
      LAM1 = 0.D0
      DO I = 0,4
        DO J = 0,5
          LAM1 = LAM1 + PROP%PROP_COEFF%LL(I,J)*(1.D0/TT-1.D0)**I*(RR-1.D0)**J
        END DO
      END DO
      LAM1 = DEXP(RR*LAM1)
      LAM2 = 0.D0
