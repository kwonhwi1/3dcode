MODULE EOS_MODULE
  ...
  CONTAINS
    ...
    SUBROUTINE IDEAL(EOS,P,T,GAS)
      ...
      GAS%RHO  = P/287.04D0/T
      GAS%H    = 1004.64D0*T
      GAS%DRDP = 1.D0/287.04D0/T
      GAS%DRDT = -P/287.04D0/T**2
      GAS%DHDP = 0.D0
      GAS%DHDT = 1004.64D0
    ...
    SUBROUTINE IAPWS97_L(EOS,P,T,LIQUID)
      ...
      DO K = 1,34
        ...
        G1 = DBLE(EOS%IAPWS_LIQUID_COEFF%J(K))*G/(TAU-1.222D0)
        G2 = - DBLE(EOS%IAPWS_LIQUID_COEFF%I(K))*G/(7.1D0-PI)
        G3 = - DBLE(EOS%IAPWS_LIQUID_COEFF%I(K)-1)*G2/(7.1D0-PI)
        G4 = DBLE(EOS%IAPWS_LIQUID_COEFF%J(K)-1)*G1/(TAU-1.222D0)
        G5 = DBLE(EOS%IAPWS_LIQUID_COEFF%J(K))*G2/(TAU-1.222D0)
        GT = GT + G1
        GP = GP + G2
        GPP = GPP + G3
        GTT = GTT + G4
      ...
      LIQUID%RHO  = 35815.9670311098D0/T/GP
      LIQUID%H    = 639675.036D0*GT
      LIQUID%DRDP = -GPP/461.526D0/T/GP**2
      LIQUID%DRDT = -LIQUID%RHO/T + 49640930.3051182D0*GPT/T**3/GP**2
      LIQUID%DHDP = 0.0386978243194192D0*GPT
    ...
    SUBROUTINE IAPWS97_V(EOS,P,T,VAPOR)
      ...
      IF(P.GT.PWW) THEN
        DO K = 1,13
          IF(K.LE.9) THEN
            ...
            G01 = DBLE(EOS%IAPWS_M_VAPOR_COEFF%J(K))*G0/TAU
            G02 = DBLE(EOS%IAPWS_M_VAPOR_COEFF%J(K)-1)*G01/TAU
            G0T = G0T + G01
            G0TT = G0TT + G02
          END IF
          GR = EOS%IAPWS_M_VAPOR_COEFF%NR(K)*PI**EOS%IAPWS_M_VAPOR_COEFF%IR(K)*(TAU-0.5D0)**EOS%IAPWS_M_VAPOR_COEFF%JR(K)
          GR1 = DBLE(EOS%IAPWS_M_VAPOR_COEFF%JR(K))*GR/(TAU-0.5D0)
          GR2 = DBLE(EOS%IAPWS_M_VAPOR_COEFF%IR(K))*GR/PI
          GR3 = DBLE(EOS%IAPWS_M_VAPOR_COEFF%IR(K)-1)*GR2/PI
          GR4 = DBLE(EOS%IAPWS_M_VAPOR_COEFF%JR(K)-1)*GR1/(TAU-0.5D0)
          GR5 = DBLE(EOS%IAPWS_M_VAPOR_COEFF%JR(K))*GR2/(TAU-0.5D0)
          GRT = GRT + GR1
          GRP = GRP + GR2
          GRPP = GRPP + GR3
          GRTT = GRTT + GR4
      ...
      ELSE
        DO K = 1,43
          IF(K.LE.9) THEN
            ...
            G01 = DBLE(EOS%IAPWS_VAPOR_COEFF%J(K))*G0/TAU
            G02 = DBLE(EOS%IAPWS_VAPOR_COEFF%J(K)-1)*G01/TAU
            G0T = G0T + G01
            G0TT = G0TT + G02
          END IF
          GR = EOS%IAPWS_VAPOR_COEFF%NR(K)*PI**EOS%IAPWS_VAPOR_COEFF%IR(K)*(TAU-0.5D0)**EOS%IAPWS_VAPOR_COEFF%JR(K)
          GR1 = DBLE(EOS%IAPWS_VAPOR_COEFF%JR(K))*GR/(TAU-0.5D0)
          GR2 = DBLE(EOS%IAPWS_VAPOR_COEFF%IR(K))*GR/PI
          GR3 = DBLE(EOS%IAPWS_VAPOR_COEFF%IR(K)-1)*GR2/PI
          GR4 = DBLE(EOS%IAPWS_VAPOR_COEFF%JR(K)-1)*GR1/(TAU-0.5D0)
          GR5 = DBLE(EOS%IAPWS_VAPOR_COEFF%JR(K))*GR2/(TAU-0.5D0)
          GRT = GRT + GR1
          GRP = GRP + GR2
          GRPP = GRPP + GR3
          GRTT = GRTT + GR4
      ...
      VAPOR%RHO  = 2166.72516824621D0/T/(G0P+GRP)
      VAPOR%H    = 249224.04D0*(G0T+GRT)
      VAPOR%DRDP = -(G0PP+GRPP)/461.526D0/T/(G0P+GRP)**2
      VAPOR%DRDT = -VAPOR%RHO/T + 1170031.59085295D0*(G0PT+GRPT)/T**3/(G0P+GRP)**2
    ...
    SUBROUTINE STIFFENED(EOS,P,T,LIQUID)
      ...
      LIQUID%RHO  = (P+8.5D8)/2691.D0/T
      LIQUID%H    = 4186.D0*T
      LIQUID%DRDP = 1.D0/2691.D0/T
      LIQUID%DRDT = -(P+8.5D8)/2691.D0/T**2
      LIQUID%DHDP = 0.D0
      LIQUID%DHDT = 4186.D0
