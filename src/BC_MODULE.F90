MODULE BC_MODULE
  ...
  CONTAINS
    ...
    SUBROUTINE SETBC(BC,GRID,VARIABLE,EOS,PROP)
      ...
      DO N=1,BC%NCON
        ...
        IF(BC%RANK.NE.BC%CONNECTINFO(N)%DONOR) THEN 
          ...
          CALL MPI_ISEND(BC%SENDBUF,L,MPI_REAL8,BC%CONNECTINFO(N)%DONOR,BC%RANK,MPI_COMM_WORLD,REQUEST2,IER)
      ...
      DO N=1,BC%NCON
        ...
        IF(BC%RANK.NE.BC%CONNECTINFO(N)%DONOR) THEN 
          ...
          CALL MPI_IRECV(BC%RECVBUF,L,MPI_REAL8,BC%CONNECTINFO(N)%DONOR,BC%CONNECTINFO(N)%DONOR,MPI_COMM_WORLD,REQUEST4,IER)
    ...
    SUBROUTINE BCCORNER(BC,GRID,VARIABLE,EOS,PROP,I,J,K,II,JJ,KK)
      ...
      IF((DQDX(2).EQ.0.D0).AND.(DQDX(3).EQ.0.D0).AND.(DQDX(4).EQ.0.D0)) THEN
        IF((DQDY(2).EQ.0.D0).AND.(DQDY(3).EQ.0.D0).AND.(DQDY(4).EQ.0.D0)) THEN
          PV = VARIABLE%GETPV(I,J)
        ELSE
          PV = VARIABLE%GETPV(I,J+JJ)
          PV(2) = -PV(2)
          PV(3) = -PV(3)
          IF(BC%NPV.GT.6) THEN
            PV(7) = -PV(7)
