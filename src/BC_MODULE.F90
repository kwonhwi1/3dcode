MODULE BC_MODULE
  ...
  TYPE T_BC
    PRIVATE
    INTEGER :: RANK,SIZE,NBC,NCON,NUMMAX
  ...
  CONTAINS
    ...
    SUBROUTINE CONSTRUCT(BC,CONFIG,GRID,VARIABLE,EOS,PROP)
      ...
      DO N=1,BC%NCON
        ...
        IF(NUM.GT.BC%NUMMAX) BC%NUMMAX = NUM
      END DO
      
      IF(BC%NCON.NE.0) THEN
        BC%NUMMAX = (BC%NPV+BC%NTV+BC%NDV)*BC%NUMMAX
        ALLOCATE(BC%SENDBUF(BC%NUMMAX),BC%RECVBUF(BC%NUMMAX))        
      END IF
    ...
    SUBROUTINE SETBC(BC,GRID,VARIABLE,EOS,PROP)
      ...
      DO N=1,BC%NCON
        ...
        IF(BC%RANK.NE.BC%CONNECTINFO(N)%DONOR) THEN 
          ...
          CALL MPI_ISEND(BC%SENDBUF,BC%NUMMAX,MPI_REAL8,BC%CONNECTINFO(N)%DONOR,BC%RANK,MPI_COMM_WORLD,REQUEST2,IER)
          ...
          DO K=BC%CONNECTINFO(N)%ISTART(3),BC%CONNECTINFO(N)%IEND(3)
            DO J=BC%CONNECTINFO(N)%ISTART(2),BC%CONNECTINFO(N)%IEND(2)
              DO I=BC%CONNECTINFO(N)%ISTART(1),BC%CONNECTINFO(N)%IEND(1)
                ...
                DO M=1,BC%NPV
                  L = L + 1
                  CALL VARIABLE%SETPV(M,II,JJ,KK,BC%SENDBUF(L))
                END DO
                DO M=1,BC%NTV
                  L = L + 1
                  CALL VARIABLE%SETTV(M,II,JJ,KK,BC%SENDBUF(L))
                END DO
                DO M=1,BC%NDV
                  L = L + 1
                  CALL VARIABLE%SETDV(M,II,JJ,KK,BC%SENDBUF(L))
                END DO
              END DO
            END DO
          END DO
      ...
      DO N=1,BC%NCON
        ...
        IF(BC%RANK.NE.BC%CONNECTINFO(N)%DONOR) THEN 
          ...
          CALL MPI_IRECV(BC%RECVBUF,BC%NUMMAX,MPI_REAL8,BC%CONNECTINFO(N)%DONOR,BC%CONNECTINFO(N)%DONOR,MPI_COMM_WORLD,REQUEST4,IER)
          ...
          DO K=RECVADRESS(5),RECVADRESS(6)
            DO J=RECVADRESS(3),RECVADRESS(4)
              DO I=RECVADRESS(1),RECVADRESS(2)
                ...
                DO M=1,BC%NPV
                  L = L + 1
                  CALL VARIABLE%SETPV(M,II,JJ,KK,BC%RECVBUF(L))
                END DO
                DO M=1,BC%NTV
                  L = L + 1
                  CALL VARIABLE%SETTV(M,II,JJ,KK,BC%RECVBUF(L))
                END DO
                DO M=1,BC%NDV
                  L = L + 1
                  CALL VARIABLE%SETDV(M,II,JJ,KK,BC%RECVBUF(L))
                END DO
              END DO
            END DO
          END DO
