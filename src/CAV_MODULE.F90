MODULE CAV_MODULE
  ...
  CONTAINS
    ...
    FUNCTION MERKLE(CAV,EOS) RESULT(MDOT)
      ...
      R_C = CAV%C_C*CAV%DV(1)*CAV%PV(6)*DMAX1(CAV%PV(1)+CAV%PREF-PWW,0.D0) &
          /(0.5D0*CAV%RHO*CAV%UREF**2)/(CAV%CHORD_LENGTH/CAV%UREF)
      R_V = CAV%C_V*CAV%DV(1)*(1.D0-CAV%PV(6)-CAV%PV(7))*DMAX1(PWW-CAV%PV(1)-CAV%PREF,0.D0) &
          /(0.5D0*CAV%RHO*CAV%UREF**2)/(CAV%CHORD_LENGTH/CAV%UREF)
      ...
      IF(R_C.NE.0.D0) THEN
        ...
        MDOT(3) = - R_C*(1.D0/CAV%PV(6)+CAV%DV(9)/CAV%DV(1))
        MDOT(4) = - R_C*CAV%DV(10)/CAV%DV(1)
      END IF
      IF(R_V.NE.0.D0) THEN
        MDOT(1) = R_V*(-1.D0/(PWW-CAV%PV(1)-CAV%PREF)+CAV%DV(7)/CAV%DV(1))
        MDOT(2) = R_V*CAV%DV(8)/CAV%DV(1)
        MDOT(3) = R_V*(CAV%DV(9)/CAV%DV(1)-1.D0/(1.D0-CAV%PV(6)-CAV%PV(7)))
        MDOT(4) = R_V*(CAV%DV(10)/CAV%DV(1)-1.D0/(1.D0-CAV%PV(6)-CAV%PV(7)))
      END IF
    ...
    FUNCTION KUNZ(CAV,EOS) RESULT(MDOT)
      ...
      IF(R_C.NE.0.D0 ) THEN
        MDOT(1) = - CAV%C_C/(CAV%CHORD_LENGTH/CAV%UREF)*(CAV%DV(7)*CAV%PV(6)*AV1+2.D0*AV**2*CAV%DV(15)*(AL+AG))
        MDOT(2) = - CAV%C_C/(CAV%CHORD_LENGTH/CAV%UREF)*(CAV%DV(8)*CAV%PV(6)*AV1+2.D0*AV**2*CAV%DV(16)*(AL+AG))
        MDOT(3) = - CAV%C_C/(CAV%CHORD_LENGTH/CAV%UREF)*(CAV%DV(9)*CAV%PV(6)+CAV%DV(1))*AV1
        MDOT(4) = - CAV%C_C/(CAV%CHORD_LENGTH/CAV%UREF)*CAV%DV(10)*CAV%PV(6)*AV1
      END IF
      IF(R_V.NE.0.D0) THEN
        MDOT(1) = R_V*(-1.D0/(PWW-CAV%PV(1)-CAV%PREF)+CAV%DV(7)/CAV%DV(1))
        MDOT(2) = R_V*CAV%DV(8)/CAV%DV(1)
    ...
    FUNCTION SINGHAL(CAV,EOS) RESULT(MDOT)
      ...
      R_C = CAV%C_C*CAV%UREF/SIGMA*CAV%DV(3)*CAV%DV(4) &
          *DSQRT(2.D0/3.D0*DMAX1(CAV%PV(1)+CAV%PREF-PWW,0.D0)/CAV%DV(3))*CAV%PV(6)
      R_V = CAV%C_V*CAV%UREF/SIGMA*CAV%DV(3)*CAV%DV(4) &
          *DSQRT(2.D0/3.D0*DMAX1(PWW-CAV%PV(1)-CAV%PREF,0.D0)/CAV%DV(3))*(1.D0-CAV%PV(6)-CAV%PV(7))
      ...
      IF(R_V.NE.0.D0 ) THEN
        ...
        MDOT(3) = - R_V*1.D0/(1.D0-CAV%PV(6)-CAV%PV(7))
        MDOT(4) = - R_V*1.D0/(1.D0-CAV%PV(6)-CAV%PV(7))
      END IF
      LAM = DSIGN(1.D0,MDOT(3))
      MDOT(1:4) = MDOT(1:4)*(PHI*(1.D0-0.5D0*(1.D0-LAM))-0.5D0*(1.D0-LAM))
